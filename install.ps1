...
function Install-Python {
    ...
    try {
        ...
        Write-Host ""
        Write-Host "[FAIL] Automatic installation failed." -ForegroundColor Red
        Write-Host ""
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "  Option 2: Run: python -m streamlit run app.py" -ForegroundColor White
    Write-Host ""
} else {
    Write-Host ""
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "[FAIL] Installation failed. Please check your internet connection." -ForegroundColor Red
    }
}
